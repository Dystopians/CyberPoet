# v13 结果码解码分析：M7RvsM7（参考模型单变量）/ M7RvsM4（对现役）+ 分臂图灵率 +
# DPO 金标行 + 反杀裱框候选 + 批注辑录。用法: python3 analyze_v13.py <码或文件>
...
for lead, other, cond, label in (("M7R","M7",lambda r: r>=0.55,"预登记：参考改回桥 ≥55%（面板碎行 14%→4.5%、短诗 52→13）"),
                                 ("M7R","M4",lambda r: r>=0.50,"预登记：对现役 ≥50%")):
...
for i, pk in picks.items():
    ...
    rows.append({"instruction": instr, "chosen": win["body"].strip(), "rejected": lose["body"].strip(),
                 "weight": w, "src": f"v13[{i}]", "chosen_src": win["src"]})
json.dump(rows, open('dpo_rows_owner_v13.json', 'w'), ensure_ascii=False, indent=1)
print(f"\nDPO 金标行: {len(rows)} 条 → dpo_rows_owner_v13.json（加权 2.0 的 {sum(1 for r in rows if r['weight']>1)} 条）")
