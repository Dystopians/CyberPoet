...
words = {}
assert 30 <= len(pairs) <= 110, len(pairs)   # v16
kinds = collections.Counter(p["kind"] for p in pairs)
assert kinds["hh"] == 0 and 12 <= kinds["ha"] <= 44 and 12 <= kinds["aa"] <= 50, kinds  # v16：ha ≤20（+标准局 ≤24）+ aa ≤50
...
from v16_arms import DUEL_A, DUEL_B, DUEL_C, HAS_W, HAS_N
...
<title>诗味测验 v16 · 散文对照卷</title><style>
...
<h1>诗 味 测 验 · 拾肆</h1>
<div class="sub">散文对照卷 · 同一配方，预训练只多加散文 · 备注即黄金</div>
<div id="app"></div>
</div><script>
const P=__PAYLOAD__;
const KEY='quiz_v16_state';
...
import os
OUT = os.environ.get('OUT', '诗味测验16_散文对照卷.html')
ARM_NOTE = ("两个 AI 同一套配方（M4 当年的），只有一处不同：M4 = 现役（一直没变，当尺子），读的诗集是 M4 当年那本；"
            "M4P = 同一本诗集再加 301 篇散文（123 万字）。M4P 对 M4 = 散文到底有没有把诗写坏。")
