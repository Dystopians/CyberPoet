# v16 盲读助手（09-21）：只读「会上卷的那几首」，读的时候不知道是哪个臂。
# 流程：① 把按匿名编号记的否决（v16_read_codes.json）经 v16_blind_key.json 对回 [槽, 臂, 种子]，写 v16_read_verdicts.json；
#       ② 跑一遍装配；③ 把装上卷的 AI 诗对回匿名编号，挑出还没读过的，写 v16_待读.txt（只有编号、题目、正文）；
#       ④ 我读完把每个编号记成 ok 或否决原因，再跑本脚本；直到「待读 0 首」为止——上卷的每一首都读过，且全程不看臂名。
# v16_read_codes.json 格式：{"ok": ["ha3|c2", ...], "veto": {"aa17|c4": "题献", ...}, "drop": ["ha5", ...]}
import json, subprocess, sys, os
KEY = json.load(open('v16_blind_key.json'))
GATED = json.load(open('v16_gen_gated.json'))
RC = json.load(open('v16_read_codes.json')) if os.path.exists('v16_read_codes.json') else {"ok": [], "veto": {}, "drop": []}
...
def write_verdicts():
    ...
    json.dump({"veto": veto, "drop": RC.get("drop", []), "force": {}}, open('v16_read_verdicts.json', 'w'), ensure_ascii=False, indent=1)
import re
for _round in range(30):   # 选集内互重（两首诗共用 10 字窗）机械处理：否决排在后面的那首 AI 诗，记入 auto，再装一遍
    write_verdicts()
    for f in ('v16_haaa_final.json', 'v16_haaa_draft.json'):
        if os.path.exists(f): os.remove(f)
    r = subprocess.run([sys.executable, 'assemble_v16_haaa.py'], capture_output=True, text=True)
    lines = (r.stdout + r.stderr).strip().splitlines()
    dup = [re.findall(r"\('(\w+)', '([AB])'\)", l) for l in lines if l.strip().startswith("- 互重")]
    if not dup or not os.path.exists('v16_haaa_draft.json'): break
    draft = {p["slot"]: p for p in json.load(open('v16_haaa_draft.json'))}
    added = 0
    for pair in dup:
...
with open('v16_待读.txt', 'w') as f:
    for code, title, body in todo: f.write(f"\n{'='*60}\n## {code} 《{title}》\n{body}\n")
print(f"已读 {len(seen)}（否决 {len(RC.get('veto', {}))}；机械否决 {len(RC.get('auto', {}))}）；本轮待读 {len(todo)} 首 → v16_待读.txt")
