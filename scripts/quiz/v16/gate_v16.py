# v16 候选门控：v16_cands_{M4P,M4sft,M4}.jsonl → v16_gen_gated.json（键 "sid|arm"）。
...
def clean(b, title=""):
    ...
    if ls and title:
        h = ls[0].strip()
        t = re.escape(title)
        # 第十卷通读所见三类回显（沿用）：「——题目」「（——题目）」「——《题目》之五」「题目：」「题目——」、首行伪日期「（1987-08-23）」
...
def gate(title, b):
    ...
    if len(re.findall(r"[A-Za-zА-Яа-яЁё]", b)) >= 3: return "Latin"   # 09-02：含西里尔（v12 漏过一首俄文）
...
for arm in ARMS:
    for l in open(f"v16_cands_{arm}.jsonl"):
        r = json.loads(l)
...
# 09-21：组内改为哈希随机序（09-16 复核：按质感降序取第一首，实际让机械分数替主人挑了 85–90% 的上卷诗）。质感与软旗照记，只供报告。
import hashlib
for k in gated: gated[k].sort(key=lambda c: hashlib.sha256(f"v16|{k}|{c['seed']}".encode()).hexdigest())
json.dump(dict(gated), open("v16_gen_gated.json", "w"), ensure_ascii=False, indent=1)
print("门控统计:", dict(stat))
cov = collections.Counter(k.split("|")[1] for k in gated)
print(f"过闸组: {len(gated)}（按臂: {dict(cov)}）→ v16_gen_gated.json")
# 覆盖缺口速报：哪些槽某臂全军覆没
