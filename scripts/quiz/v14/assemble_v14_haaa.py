# v14 装配 · ha≤24 + aa≤56。臂位/对决按 v14_slot_proposal.json（读稿前哈希预分配，防看质量挑臂）；
# 通读否决按 v14_read_verdicts.json（盲读：否决先按匿名编号记，再经 v14_blind_key.json 对回 [槽, 臂, 种子]）。本卷无 hh。
...
PLAN = json.load(open('v14_slot_proposal.json'))
RV = json.load(open('v14_read_verdicts.json'))
import hashlib
...
QUOTA_AA = {"M8vsM4": 36, "M8vsM8sft": 20}
pairs, log = [], []
...
def try_slot(sid, duel):
    a1, a2 = duel.split("vs")
    first, second = (a1, a2) if int(hashlib.sha256(("v14first" + sid).encode()).hexdigest(), 16) % 2 == 0 else (a2, a1)
...
if errs:
    json.dump(out, open('v14_haaa_draft.json', 'w'), ensure_ascii=False, indent=1)
    print("!! 待处理:"); [print("  -", e) for e in errs]
else:
    json.dump(out, open('v14_haaa_final.json', 'w'), ensure_ascii=False, indent=1)
    hu = collections.Counter("A" if p["A"]["src"] == "human" else "B" for p in out if p["kind"] == "ha")
    print(f"OK: ha {len(ha_pairs)}(人侧 {dict(hu)}) + aa {len(aa_pairs)} → v14_haaa_final.json")
    for duel in QUOTA_AA:
