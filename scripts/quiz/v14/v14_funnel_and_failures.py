# v14 交付附件（09-22，Codex 评审 C1/C9 + 主人 09-21 「别让我在测验里替你数病」）：
# ① 完整漏斗：每臂 原始候选 → 硬门控（逐条拒因）→ 盲读否决 → 上卷；② 出厂面板与候选的可数体检（循环/字数/短诗/撞顶 + 文本模式占比）；
# ③ 每臂最常见三类失败的原文短摘（≤2 行，标明 AI 生成，只摘不评）。输出 v14_交付附件_漏斗与失败.md。用法：候选齐、门控与装配跑完后执行。
import json, re, collections, os, sys
sys.path.insert(0, '.')
from v14_arms import ARMS
from gate_v14 import clean, gate
...
gated = json.load(open('v14_gen_gated.json')) if os.path.exists('v14_gen_gated.json') else {}
final = json.load(open('v14_pairs_final.json')) if os.path.exists('v14_pairs_final.json') else []
rv = json.load(open('v14_read_verdicts.json')) if os.path.exists('v14_read_verdicts.json') else {"veto": []}
...
out = ["# v14 交付附件：候选漏斗与失败短摘（09-22）", "", "只数不评；短摘全是 AI 生成，人诗一字不引。", ""]
out += ["## 一、漏斗（每臂）", "", "| 臂 | 原始候选 | 过硬门控 | 门控拒因（前三） | 盲读否决 | 上卷 |", "|---|---|---|---|---|---|"]
stats = {}
for arm in ARMS:
    f = f"v14_cands_{arm}.jsonl"
...
for arm, (rows, cl, ex) in stats.items():
    ...
    out.append("")
open('v14_交付附件_漏斗与失败.md', 'w', encoding='utf-8').write("\n".join(out))
print("\n".join(out[:14])); print("→ v14_交付附件_漏斗与失败.md")
