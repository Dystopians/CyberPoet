# v14 候选门控：v14_cands_{M8,M8sft,M4}.jsonl → v14_gen_gated.json（键 "sid|arm"）。
...
import hashlib
for k in gated: gated[k].sort(key=lambda c: hashlib.sha256(f"v14|{k}|{c['seed']}".encode()).hexdigest())
json.dump(dict(gated), open("v14_gen_gated.json", "w"), ensure_ascii=False, indent=1)
print("门控统计:", dict(stat))
cov = collections.Counter(k.split("|")[1] for k in gated)
print(f"过闸组: {len(gated)}（按臂: {dict(cov)}）→ v14_gen_gated.json")
# 覆盖缺口速报：哪些槽某臂全军覆没
