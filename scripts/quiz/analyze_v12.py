# v12 结果码解码分析：M7vsM7F（同数据全参对 LoRA）/ M7FvsM7Fsft（dpo_v1 对新底座）+ 分臂图灵率 + 定案局诗人级判定 +
# DPO 金标行 + 反杀裱框候选 + 批注辑录。用法: python3 analyze_v12.py <码或文件>
...
for lead, other, cond, label in (("M7","M7F",lambda r: True,"无预测（全参对 LoRA 首次人读）"),
                                 ("M7Fsft","M7F",lambda r: r>=0.50,"预登记：未做 DPO 不劣于做了 dpo_v1(≥50%)——面板显示 dpo_v1 把新底座拽平")):
...
for i, pk in picks.items():
    ...
    rows.append({"instruction": instr, "chosen": win["body"].strip(), "rejected": lose["body"].strip(),
                 "weight": w, "src": f"v12[{i}]", "chosen_src": win["src"]})
json.dump(rows, open('dpo_rows_owner_v12.json', 'w'), ensure_ascii=False, indent=1)
print(f"\nDPO 金标行: {len(rows)} 条 → dpo_rows_owner_v12.json（加权 2.0 的 {sum(1 for r in rows if r['weight']>1)} 条）")
