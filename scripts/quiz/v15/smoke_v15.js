// v15 页面冒烟：正常答完 / 全跳过 / 空卷 / 结果码导入与退出 / 错版码拒收
// 运行：node smoke_v75.js 诗味测验7点5_真人对决卷.html
const fs = require('fs');
const file = process.argv[2] || '诗味测验15_M4对照卷.html';
...
const N = P.length;
console.log('题数:', N);
const EXP = JSON.parse(require('fs').readFileSync('v15_pairs_final.json', 'utf8')).length;
...
if (decoded.v !== 15) throw '结果码版本不是 15: ' + decoded.v;
...
// ---- 5) 空卷直接出结果 ----
store = { quiz_v15_state: JSON.stringify({ nick: '', i: 999, picks: {}, marks: {} }) };
freshRun();
eval(js);
