# v15 装配 · ha≤24 + aa≤56。臂位/对决按 v15_slot_proposal.json（读稿前哈希预分配，防看质量挑臂）；
# 通读否决按 v15_read_verdicts.json（盲读：否决先按匿名编号记，再经 v15_blind_key.json 对回 [槽, 臂, 种子]）。本卷无 hh。
...
PLAN = json.load(open('v15_slot_proposal.json'))
RV = json.load(open('v15_read_verdicts.json'))
import hashlib
...
from v15_arms import DUEL_A, DUEL_B, DUEL_C, QUOTA_HA, QUOTA_AA, ARMS   # v15：ha M4R 12 / M4sft 6 / M4 6；aa 30/15/15
pairs, log = [], []
...
for sid in rest:
    ...
    chosen[sid] = opts[0]
assert len(chosen) >= 20, f"ha 只凑到 {len(chosen)}"
...
def try_slot(sid, duel):
    a1, a2 = duel.split("vs")
    first, second = (a1, a2) if int(hashlib.sha256(("v15first" + sid).encode()).hexdigest(), 16) % 2 == 0 else (a2, a1)
...
hh = json.load(open('v15_hh_pairs.json'))   # v15 有 hh 24：ha/aa 与之互查 10 字窗与同题
def fg(t):
    s = norm(t); return {s[i:i+10] for i in range(max(len(s)-9, 1))}
...
if errs:
    json.dump(out, open('v15_haaa_draft.json', 'w'), ensure_ascii=False, indent=1)
    print("!! 待处理:"); [print("  -", e) for e in errs]
else:
    json.dump(out, open('v15_haaa_final.json', 'w'), ensure_ascii=False, indent=1)
    hu = collections.Counter("A" if p["A"]["src"] == "human" else "B" for p in out if p["kind"] == "ha")
    print(f"OK: ha {len(ha_pairs)}(人侧 {dict(hu)}) + aa {len(aa_pairs)} → v15_haaa_final.json")
    for duel in QUOTA_AA:
