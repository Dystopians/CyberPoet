...
words = {}
assert 70 <= len(pairs) <= 108, len(pairs)   # v15
kinds = collections.Counter(p["kind"] for p in pairs)
assert kinds["hh"] == 24 and 18 <= kinds["ha"] <= 24 and 40 <= kinds["aa"] <= 60, kinds  # v15：hh 24 + ha 24 + aa 30/15/15
...
from v15_arms import DUEL_A, DUEL_B, DUEL_C, HAS_W, HAS_N
...
<title>诗味测验 v15 · M4 对照卷</title><style>
...
<h1>诗 味 测 验 · 拾肆</h1>
<div class="sub">M4 对照卷 · 同一个底子三种做法 · 真人对真人 24 题选下一卷的标准诗 · 备注即黄金</div>
<div id="app"></div>
</div><script>
const P=__PAYLOAD__;
const KEY='quiz_v15_state';
...
import os
OUT = os.environ.get('OUT', '诗味测验15_M4对照卷.html')
ARM_NOTE = ("这卷四种诗：真人对真人 24 题（陆忆敏、杨炼、昌耀、张枣、多多、商禽各 8 首，为下一卷『标准局』选你喜欢的真人诗）；"
            "AI 对真人 24 题；AI 对 AI 60 题。三个 AI 都是 M4 的底子：M4 = 现役（一直没变，当尺子）；"
            "M4R = M4 的底子，最后一步『教它你喜欢哪种』换成改过的做法；M4sft = M4 的底子，不做最后一步。"
            "M4R 对 M4 = 改过的做法有没有用；M4 对 M4sft = M4 当年那一步有没有用；M4R 对 M4sft = 改过的做法有没有改变底子。")
