# v15 候选门控：v15_cands_{M4R,M4sft,M4}.jsonl → v15_gen_gated.json（键 "sid|arm"）。
...
for arm in ARMS:
    for l in open(f"v15_cands_{arm}.jsonl"):
        r = json.loads(l)
...
import hashlib
for k in gated: gated[k].sort(key=lambda c: hashlib.sha256(f"v15|{k}|{c['seed']}".encode()).hexdigest())
json.dump(dict(gated), open("v15_gen_gated.json", "w"), ensure_ascii=False, indent=1)
print("门控统计:", dict(stat))
cov = collections.Counter(k.split("|")[1] for k in gated)
print(f"过闸组: {len(gated)}（按臂: {dict(cov)}）→ v15_gen_gated.json")
# 覆盖缺口速报：哪些槽某臂全军覆没
