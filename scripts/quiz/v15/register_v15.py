# v15 登记：把上卷的每一首（人、机两侧）记进 used_bodies.json，供以后各卷 10 字窗查重。
# 与 v13 做法相同：先备份 used_bodies_pre_v15.json；只加不删；重复执行无副作用。
import json, os, sys
sys.path.insert(0, '.')
from v5_lib import norm
used = json.load(open('used_bodies.json'))
if not os.path.exists('used_bodies_pre_v15.json'):
    json.dump(used, open('used_bodies_pre_v15.json', 'w'), ensure_ascii=False)
