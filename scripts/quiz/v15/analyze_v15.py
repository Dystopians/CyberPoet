# v15 结果码解码分析（09-22，实验 1「M4R」）：头条 = 同卷对 M4（M4 是尺子）；三对决 M4RvsM4 / M4RvsM4sft / M4vsM4sft；hh 24 选标准诗；
# DPO 金标行 + 批注辑录。用法: python3 analyze_v15.py <码或文件>
import json, sys, base64, math, collections, re
from v5_lib import tags, norm
from v15_arms import LEAD_B, DUEL_C, MAIN
...
def duel_stat(lead, other):
    ...
    return w, n, x
for lead, other, what in ((MAIN, "M4", "改过的最后一步 对 现役"), ("M4sft", "M4", "不做最后一步 对 现役"), (MAIN, "M4sft", "改过的最后一步 对 不做")):
...
print("aa 臂间对决（M4R 对 M4 / M4R 对 M4sft；预登记见下；45–55% 记均衡，决定票 <30 只记不判）:")
for (x, y), (wx, n) in sorted(duel.items()):
    print(f"  {x} {wx} : {n-wx} {y}   p={binom2(wx,n):.3f}" + ("  **" if binom2(wx,n) < 0.05 else ""))

# ---- 1b) 预登记对账（v15 制卷记录，作答前锁定）----
print("\n预登记对账（判决只认票；n<30 只记不判；45–55% 记均衡）:")
PRE = ((MAIN, "M4", None, "不预测——这是本卷的问题。读法：≥55% 改过的做法有用；≤45% 偏好训练这条线放下；45–55% 偏好训练在你眼里没作用"),
       ("M4", "M4sft", lambda r: r > 0.55, "预登记：M4 >55%（M4 当年那一步有用；若均衡 → M4 的强项全在底子里，最后一步无关）"),
       (MAIN, "M4sft", None, "不预测：改过的做法有没有改变桥"))
...
json.dump(std, open('v15_hh_winners.json', 'w'), ensure_ascii=False, indent=1)
print(f"  你选中的真人诗 {len(std)} 首 → v15_hh_winners.json（下一卷标准局的候选）；都不要 {sum(1 for i, pk in picks.items() if pairs[i]['kind'] == 'hh' and pk == 'X')} 局")
...
for i, pk in picks.items():
    ...
    rows.append({"instruction": instr, "chosen": win["body"].strip(), "rejected": lose["body"].strip(),
                 "weight": w, "src": f"v15[{i}]", "chosen_src": win["src"]})
json.dump(rows, open('dpo_rows_owner_v15.json', 'w'), ensure_ascii=False, indent=1)
print(f"\nDPO 金标行: {len(rows)} 条 → dpo_rows_owner_v15.json（加权 2.0 的 {sum(1 for r in rows if r['weight']>1)} 条）")
