...
# 09-21 21:55 重抽（v14b）：M8 的桥改用上一代 sft_pt9b（桥数据 v4），出卷题与金标必须同时避开 v4 与 v5 的训练题面和目标正文（第一次抽样只避了 v5，结果 40 首金标里 38 首在 v4 里、84 道命题 71 道在 v4 里）。
TRAIN_TITLES, TRAIN_GRAMS = set(), set()
for _f in ("../data/sft_train_v4.json", "../data/sft_train_v5.json"):
    for _r in json.load(open(_f)):
        _m = re.search(r"《(.+?)》", _r["instruction"])
        if _m and ("为题" in _r["instruction"] or "题为" in _r["instruction"]): TRAIN_TITLES.add(_m.group(1).strip())
        TRAIN_GRAMS |= full_grams(_r["output"])
        if _r.get("input"): TRAIN_GRAMS |= full_grams(_r["input"])
_ok0 = ok
def ok(p, need_title=True):
