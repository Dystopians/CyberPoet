...
# 协议：① 预热占位进程先挂在候选卡上（只占约 0.45 GB），卡一空出来就瞬间占 7 GiB → 对方按自己的规则排队，不会被撞死；
...
snipe_launch(){   # $1 序号  其余=卡列表
  ...
  for c in "$@"; do
    CUDA_VISIBLE_DEVICES=$c nohup $PYV $N/gpu_sniper.py $c ${SNIPE_NEED[$c]} 7 $SNIPE_DIR/$n.claim $SNIPE_DIR/$n.release 1200 >> $SNIPE_DIR/$n.card$c.log 2>&1 &
  done
