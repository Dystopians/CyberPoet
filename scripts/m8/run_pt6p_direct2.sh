#!/bin/bash
# 实验 2「pt6+散文」链 · 直起版（09-22 23:50：1 号卡整卡空出，第一段直接上；后续段 acquire_big，候选卡已加 1/2 号）（原 09-22 20:30，主人「按你说的做」）：M4 的配方一字不改，只把 pt9 的 301 篇散文加进预训练。
...
train_stage(){   # $1 名字(=输出目录名)  $2 配置
  for attempt in 1 2 3 4; do
    ...
    rm -rf $N/outputs/$1
    if [ -n "$FORCE_CARD" ] && [ "$(nvidia-smi --query-gpu=memory.free --format=csv,noheader,nounits -i $FORCE_CARD | tr -d " ")" -ge $NEED ]; then card=$FORCE_CARD; log "直接用 $card 号卡（空余 $(nvidia-smi --query-gpu=memory.free --format=csv,noheader,nounits -i $card | tr -d " ") MiB，不等占位）"; FORCE_CARD=""; else card=$(acquire_big); fi
    # 17:25 修：大任务（bf16 ≥34 GB）装载前先放掉这张卡上的占位——占位 auto 模式会把整卡多出来的都持住（这次 24.5 GB），装载会 OOM；装好后 loaded_rearm 再挂
    n=$(cat $SNIPE_DIR/current 2>/dev/null || echo 0); [ -f $SNIPE_DIR/$n.claim ] && { touch $SNIPE_DIR/$n.release; sleep 5; }
    log "$1 第 $attempt 次起跑（卡 $card，空余 $(nvidia-smi --query-gpu=memory.free --format=csv,noheader,nounits -i $card | tr -d " ") MiB）"
...
log "起（23:50 直起版：1 号卡整卡空出，不再等 M4 候选；FORCE_CARD=$FORCE_CARD）"
