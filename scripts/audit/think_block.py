"""实验 D：推理用 HF 模板(含空 think 块) 与 训练格式(无 think 块) 的差异。M4 同题同种子。"""
...
for fmt in ("hf_think","train_fmt"):
    for i in range(0,len(titles),8):
        ...
        with torch.no_grad():
            o=model.generate(**enc,max_new_tokens=560,do_sample=True,temperature=0.9,top_p=0.9,repetition_penalty=1.08,pad_token_id=tok.pad_token_id)
        for t,sq in zip(chunk,o):
            body=tok.decode(sq[enc["input_ids"].shape[1]:],skip_special_tokens=True).strip()
            out.write(json.dumps({"fmt":fmt,"title":t,"body":body},ensure_ascii=False)+"\n")
        out.flush(); print(fmt,i,flush=True)
